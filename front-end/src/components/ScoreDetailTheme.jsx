...
export default class ScoreDetailTheme extends React.Component {
  ...
  render() {
    ...
    return (
      <div className={"score-detail-theme" + ((theme.multiplier > 0)? ' score-detail-theme-complete': '')}>
        <div className="score-detail-theme-name">{theme.name}</div>
        <div className="score-detail-theme-amount">{theme.amount + '/' + theme.reqCount +
        ((theme.multiplier > 0)? ` (${theme.baseScore} x ${theme.multiplier})`: '')}</div>
              ...
              return <div className="score-detail-theme-item inventory-slot-spawn" style={{"--slot-spawn-delay": ((0.1 * idx) + 's')}}>
                {
                  (item.amount > 0)?
                    <div className="score-detail-theme-item-inner">
                      <img className="score-detail-theme-item-img" src={Utils.getItemImage(item.itemId)}/>
